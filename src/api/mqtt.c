...
FbMqtt *
fb_mqtt_new(GSocketClient *gc)
{
	FbMqtt *mqtt;
	FbMqttPrivate *priv;

    g_return_val_if_fail(G_IS_SOCKET_CLIENT(gc), NULL);
...
fb_mqtt_close(FbMqtt *mqtt)
{
	...
	if (priv->wev > 0) {
        g_source_remove(priv->wev);
		priv->wev = 0;
	}

	if (priv->rev > 0) {
        g_source_remove(priv->rev);
		priv->rev = 0;
	}
    ...
    if (priv->gsc != NULL) {
        g_io_stream_close((GIOStream*)priv->gsc, NULL, NULL);
        g_object_unref(priv->gsc);
        priv->gsc = NULL;
    }

    if(priv->ch != NULL) {
        g_io_channel_unref(priv->ch);
        priv->ch = NULL;
    }
...
static gboolean
fb_mqtt_cb_read(GIOChannel *channel, GIOCondition cond, gpointer data)
{
	FbMqtt *mqtt = data;
	FbMqttMessage *msg;
	FbMqttPrivate *priv = mqtt->priv;
	...
	gsize size;
	gssize rize;
    GInputStream *stream = g_io_stream_get_input_stream((GIOStream*)priv->gsc);
...
static gboolean
fb_mqtt_cb_write(GIOChannel *source, GIOCondition cond, gpointer data)
{
	FbMqtt *mqtt = data;
	FbMqttPrivate *priv = mqtt->priv;
	gssize wize;

    GOutputStream *stream = g_io_stream_get_output_stream((GIOStream*)priv->gsc);
    wize = g_output_stream_write(stream, priv->wbuf->data, priv->wbuf->len, NULL, NULL);
...
fb_mqtt_write(FbMqtt *mqtt, FbMqttMessage *msg)
{
    ...
    fb_mqtt_cb_write(priv->ch, G_IO_OUT, mqtt);

    if (priv->wev > 0) {
        priv->wev = g_io_add_watch(priv->ch, G_IO_OUT, fb_mqtt_cb_write, mqtt);
    }
}
...
fb_mqtt_cb_open(gpointer data)
{
	...
	fb_mqtt_timeout_clear(mqtt);
    priv->rev = g_io_add_watch(priv->ch, G_IO_IN, fb_mqtt_cb_read, mqtt);
...
fb_mqtt_open(FbMqtt *mqtt, const gchar *host, gint port)
{
	...
	fb_mqtt_close(mqtt);
    priv->gsc = g_socket_client_connect_to_host(priv->gc, host, port, NULL, &err);
    ...
    GSocket *socket = g_socket_connection_get_socket(priv->gsc);
    priv->ch = g_io_channel_unix_new(g_socket_get_fd(socket));

    fb_mqtt_cb_open(mqtt);
	fb_mqtt_timeout(mqtt);
}
